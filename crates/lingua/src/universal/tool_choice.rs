...
while extracting canonical fields (`mode`, `tool_name`) for cross-provider conversion.
...
// Note: parallel_tool_calls is passed explicitly for Anthropic's disable_parallel_tool_use
...
fn from_openai_chat(value: &Value) -> ToolChoiceConfig {
    match value {
        Value::String(s) => {
            let mode = s.parse().ok();
            ToolChoiceConfig {
...
fn from_openai_responses(value: &Value) -> ToolChoiceConfig {
    match value {
        Value::String(s) => {
            let mode = s.parse().ok();
            ToolChoiceConfig {
        ...
        Value::Object(obj) => {
            // Responses API uses flatter structure: { type: "function", name: "..." }
...
fn from_anthropic(value: &Value) -> ToolChoiceConfig {
...
fn to_openai_chat(config: &ToolChoiceConfig) -> Option<Value> {
    // If we have raw and it came from OpenAI Chat format, prefer it
    if let Some(ref raw) = config.raw {
        // Check if it looks like OpenAI Chat format (has nested function object or is string)
        if raw.is_string()
            || raw
                .as_object()
                .map_or(false, |o| o.contains_key("function"))
        {
...
fn to_openai_responses(config: &ToolChoiceConfig) -> Option<Value> {
    // If we have raw and it looks like Responses format, prefer it
    if let Some(ref raw) = config.raw {
        // Responses format has flat structure (name at top level, not nested in function)
        if raw.is_string()
            || raw.as_object().map_or(false, |o| {
...
fn to_anthropic(config: &ToolChoiceConfig, parallel_tool_calls: Option<bool>) -> Option<Value> {
    // If we have raw and it looks like Anthropic format, prefer it
    // But we may need to merge in disable_parallel_tool_use from params
    if let Some(ref raw) = config.raw {
        if raw.as_object().map_or(false, |o| {
            o.get("type")
                .and_then(Value::as_str)
                .map_or(false, |t| ["auto", "any", "none", "tool"].contains(&t))
        }) {
            // If parallel_tool_calls is explicitly false, we need to add disable_parallel_tool_use
    ...
    obj.insert(
        "type".into(),
        Value::String(mode.as_anthropic_str().into()),
    );
    ...
    if parallel_tool_calls == Some(false) || config.disable_parallel == Some(true) {
        obj.insert("disable_parallel_tool_use".into(), Value::Bool(true));
    }
...
mod tests {
    ...
    fn test_to_openai_chat_auto() {
        ...
        let value = config.to_provider(ProviderFormat::OpenAI, None).unwrap().unwrap();
    ...
    fn test_to_anthropic_any() {
        ...
        let value = config.to_provider(ProviderFormat::Anthropic, None).unwrap().unwrap();
    ...
    fn test_to_anthropic_with_parallel_disabled() {
        ...
        let value = config.to_provider(ProviderFormat::Anthropic, Some(false)).unwrap().unwrap();
    ...
    fn test_roundtrip_openai_chat() {
        ...
        let back = config.to_provider(ProviderFormat::OpenAI, None).unwrap().unwrap();
    ...
    fn test_cross_provider_openai_to_anthropic() {
        ...
        let anthropic_value = config.to_provider(ProviderFormat::Anthropic, None).unwrap().unwrap();
