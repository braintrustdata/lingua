...
async fn main() -> anyhow::Result<()> {
    ...
    let result = router.handle(body, Some(ProviderFormat::OpenAI), None).await?;
...
let bytes = router.complete(body, "gpt-4", Some(ProviderFormat::OpenAI), None).await?;
let stream = router.complete_stream(body, "gpt-4", Some(ProviderFormat::OpenAI), None).await?;
```
