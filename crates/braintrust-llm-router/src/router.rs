...
mod tests {
  ...
  "publishers/anthropic/models/claude-sonnet-4-6": {
    ...
    #[test]
    fn failover_routes_use_equivalent_provider_native_bedrock_model() {
        let model = "claude-sonnet-4-6";
        let bedrock_model = "us.anthropic.claude-sonnet-4-6";
        let catalog = ModelCatalog::from_json_str(
            r#"{
  "claude-sonnet-4-6": {
    "format": "anthropic",
  ...
  "us.anthropic.claude-sonnet-4-6": {
    ...
    "available_providers": ["bedrock"]
        ...
        let routes = router
            ...
            .expect("failover routes resolve");
        let route_info: Vec<(&str, &str, ProviderFormat)> = routes
            .iter()
            .map(|route| (route.provider_alias(), route.model(), route.format))
            .collect();

        assert_eq!(
            route_info,
            vec![
                ("anthropic", model, ProviderFormat::Anthropic),
                ("bedrock", bedrock_model, ProviderFormat::Converse),
            ]
        );
    }
