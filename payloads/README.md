...
Just updating snapshots with `make test-payloads UPDATE=1` would make tests pass without verifying the new format works with real APIs.
...
| **Just snapshot formatting changes** | `make test-payloads UPDATE=1` | No logic changed, just update snapshot formatting |
