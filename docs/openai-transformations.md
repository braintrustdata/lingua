...
| --- | --- | --- | --- | --- | --- |
| Vertex routing & model rewrite | `fetchOpenAI` 1871-1898 | Rewrite Vertex URLs and strip `publishers/*/models/` prefixes based on endpoint | `transformations::apply_model_normalization` | Complete (payload only) | URL routing remains in braintrust-llm-router; model name normalization in payload implemented |
...
| Remove `stream_options` for unsupported providers | `fetchOpenAI` 1975-1981 | Drop `stream_options` for Mistral, Fireworks, Databricks | `transformations::apply_provider_sanitization` | Complete | Prevents provider errors |
| Remove `parallel_tool_calls` for unsupported providers | `fetchOpenAI` 1983-1990 | Drop parallel tool calls for Mistral, Databricks, Azure | `transformations::apply_provider_sanitization` | Complete | Keeps compatibility with provider limits |
| Delete `seed` in Azure Entra flow | `fetchOpenAI` 1995-1999 | Remove unsupported `seed` when Azure API version is set | `transformations::apply_provider_sanitization` | Complete | Required for Azure REST validation |
...
| Responses API auto-pivot for pro models | `fetchOpenAI` 2058-2068 | Send specific pro models via Responses API | `transformations::check_responses_api_routing` | Complete | Applies to `o1-pro`, `o3-pro`, `gpt-5-pro`, `gpt-5-codex` |
